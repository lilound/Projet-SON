  ...
  if (Serial.available() > 0) {
    ...
    if (commande == "START_DIAG") {
      ...
      if (commande.indexOf(';') > 0){
        int ind = commande.indexOf(';');
        if (ind < 0) return;
        String msg = commande.substring(0, ind);

        
        if(msg == "START_CORR"){
          modeCorrection = true;
          int iCor = commande.indexOf(';', ind + 1);
          if (iCor < 0) return;
          String correction = commande.substring(ind + 1, iCor);
          mettreAJourFiltresCorrection(correction);
        }
        else{
...
void mettreAJourFiltresCorrection(String commande) {
  unsigned int startIndex = 0;
  
  // Ta chaîne est : "125.00,70.00,10.00,250.00,..."
  // On boucle 7 fois car on a 7 fréquences
  for (int i = 0; i < 7; i++) {
    
    // 1. Extraire la Fréquence (on l'ignore pour le réglage des gains, 
    // car l'ordre i correspond déjà à level_0...6)
    int comma1 = commande.indexOf(',', startIndex);
    // float freq = commande.substring(startIndex, comma1).toFloat();
    
    // 2. Extraire la Perte GAUCHE
    int comma2 = commande.indexOf(',', comma1 + 1);
    float perteG = commande.substring(comma1 + 1, comma2).toFloat();
    
    // 3. Extraire la Perte DROITE
    int comma3 = commande.indexOf(',', comma2 + 1);
    if (comma3 == -1) comma3 = commande.length(); // Pour la dernière valeur
    float perteD = commande.substring(comma2 + 1, comma3).toFloat();

    // CALCUL DU GAIN DE CORRECTION
    // Si la perte est de 70dB, on applique un gain (ex: perte/2 ou perte/3)
    // Ici j'utilise ton calcul gain = perte/3
    float gainG = perteG / 20.0;
    float gainD = perteD / 20.0;

    // ENVOI AUX FILTRES FAUST
    // level_0 = GAUCHE, level_1 = DROITE
    myFilters.setParamValue(("/Filters/level_0" + String(i)).c_str(), gainG);
    myFilters.setParamValue(("/Filters/level_1" + String(i)).c_str(), gainD);
    
    // On déplace l'index pour la prochaine fréquence
    startIndex = comma3 + 1;
    if (startIndex >= commande.length()) break;
  }
  ...
  modeDiagnostic = false;
  myDsp.setMute(false);
  myDsp.setDiagnostic(false); // On repasse sur le micro
  
  Serial.println("CORRECTION_APPLIED");
}
